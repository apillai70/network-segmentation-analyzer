...
def cleanup_tracking():
    ...
    print("📋 Cleaning up file tracking...")
...
def cleanup_outputs():
    ...
    if word_dir.exists():
        ...
        print(f"  ✓ Deleted: outputs_final/word_reports/")
...
def cleanup_input_files():
    ...
    if input_dir.exists():
        ...
        if app_list.exists():
            print(f"  ✓ Kept: applicationList.csv")
...
def move_file_to_input(source_file):
    ...
    if not source.name.startswith('App_Code_'):
        print(f"  ⚠️  Warning: File doesn't follow App_Code_*.csv naming")
...
def verify_clean_state():
    ...
    if app_files:
        print(f"\n📋 Found {len(app_files)} file(s) ready to process:")
        for file in app_files:
            print(f"    - {file.name}")
    else:
        print(f"\n📋 No App_Code_*.csv files in data/input/ yet")

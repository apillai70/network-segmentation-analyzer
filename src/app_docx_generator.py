...
class ApplicationArchitectureDocument:
    ...
    def _add_diagram_page(self, png_path: str):
        """Add page with embedded diagram"""
        ...
        if png_file.exists():
            try:
                # Add image with appropriate width (6.5 inches for letter size with margins)
                self.doc.add_picture(str(png_file), width=Inches(6.5))
                logger.info(f"  PNG embedded: {png_file.name}")
